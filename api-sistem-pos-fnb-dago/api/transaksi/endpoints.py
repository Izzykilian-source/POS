...
def get_riwayat_transaksi():
    ...
    cursor = None
    try:
        # 1. Ambil identitas dari token
        import json
        identity = get_jwt_identity()
        
        # --- PERBAIKAN LOGIKA PENGAMBILAN USER ID ---
        if isinstance(identity, dict):
            user_id = identity.get('id_user')
        elif isinstance(identity, str):
            try:
                # Jika token berupa string JSON
                parsed_identity = json.loads(identity)
                user_id = parsed_identity.get('id_user') if isinstance(parsed_identity, dict) else parsed_identity
            except:
                # Jika token langsung berupa string ID (misal: "123")
                user_id = identity
        else:
            user_id = identity
        # --------------------------------------------
        ...
        if not user_id:
            return jsonify({"message": "User ID tidak ditemukan dalam token"}), 401

        connection = get_connection()
        cursor = connection.cursor(dictionary=True)
        
        # ... (Sisa kode query Anda di bawahnya tetap biarkan saja) ...
