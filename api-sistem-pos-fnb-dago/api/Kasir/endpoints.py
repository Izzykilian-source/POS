...
def get_saved_order_detail(id_transaksi):
    ...
    untuk order yang disimpan ATAU pesanan dari pelanggan (Belum Lunas).
    """
    connection = None
    cursor = None
    try:
        ...
        # ✅ PERBAIKAN: Mengizinkan status 'Disimpan' DAN 'Belum Lunas'
        cursor.execute("""
            SELECT
            ...
            FROM transaksi
            WHERE id_transaksi = %s AND status_pembayaran IN ('Disimpan', 'Belum Lunas')
        """, (id_transaksi,))
        
        transaksi_data = cursor.fetchone()

        if not transaksi_data:
            return jsonify({"message": "ERROR", "error": "Order tersimpan tidak ditemukan atau sudah dibayar."}), 404

        # 2. Ambil detail item F&B
        cursor.execute("""
            SELECT
...
def pay_saved_order(id_transaksi):
    """
    Memproses pembayaran untuk order yang sebelumnya disimpan (Draft) 
    atau pesanan masuk dari pelanggan (Belum Lunas).
    """
    jwt_identity = get_jwt_identity()
    try:
        id_user_kasir = jwt_identity.get('id_user')
    ...
    try:
        data = request.get_json()
        if not data: 
            return jsonify({"message": "ERROR", "error": "Data tidak valid"}), 400

        id_sesi_aktif = data.get('id_sesi')
        connection = get_connection()
        cursor = connection.cursor(dictionary=True)
        
        # 1. Validasi Sesi Kasir
        cursor.execute("SELECT id_sesi FROM sesi_kasir WHERE id_sesi = %s AND status_sesi = 'Dibuka' LIMIT 1", (id_sesi_aktif,))
        if not cursor.fetchone():
            return jsonify({"message": "ERROR", "error": "Sesi kasir tidak valid atau sudah ditutup."}), 403
        ...
        metode_pembayaran_db = payment_map.get(payment_method_raw, "Tunai")
        ...
        if existing_order['status_pembayaran'] == 'Lunas':
            return jsonify({"message": "ERROR", "error": "Transaksi ini sudah lunas."}), 400
        ...
        # 6. UPDATE TRANSAKSI MENJADI LUNAS
        query_update = """
        ...
        # Respon sukses tanpa embel-embel voucher
        return jsonify({
            "message": "OK",
            "info": f"Order #{id_transaksi} berhasil dibayar lunas.",
        ...
        return jsonify({"message": "ERROR", "error": f"Kesalahan Sistem: {str(e)}"}), 500
    finally:
        if cursor: cursor.close()
        if connection: connection.close()
...
def readTransaksiKasirs():
    ...
    try:
        ...
        # --- LOGIKA AMAN PENGAMBILAN ID (ANTI-GAGAL) ---
        if isinstance(identity, dict):
            current_kasir_id = identity.get('id_user', identity.get('id'))
        elif isinstance(identity, str):
            try:
                # Coba parse siapa tau ini string JSON
                parsed_identity = json.loads(identity)
                if isinstance(parsed_identity, dict):
                    current_kasir_id = parsed_identity.get('id_user', parsed_identity.get('id'))
                else:
                    current_kasir_id = parsed_identity
            except:
                # Kalau gagal di-parse, berarti emang string murni (misal: "115")
                current_kasir_id = identity
        else:
            # Jika dia tipe data lain (misal langsung Integer 115)
            current_kasir_id = identity
            
        # Pastikan tidak None
        if not current_kasir_id:
            return jsonify({"message": "ERROR", "error": "Invalid token format, no user ID found."}), 401
        # ------------------------------------------------
        ...
        # --- PERUBAHAN BARU: AMBIL DATA VOUCHER (DIMATIKAN SEMENTARA) ---
...
def readMerchantOrders():
    """Ambil daftar order khusus F&B (merchant kasir) HARI INI SAJA"""
    connection = None
    cursor = None
    try:
        connection = get_connection()
        ...
        # 1. PERBARUI QUERY: Tambahkan WHERE untuk memfilter hari ini
        query = """
        SELECT 
            t.id_transaksi,
            COALESCE(u.nama, t.nama_guest) AS customer_name,
        ...
        -- ✅ INI KUNCI PERBAIKANNYA: Hanya ambil transaksi hari ini
        WHERE DATE(t.tanggal_transaksi) = CURDATE()
